...
class startmenu():
    ...
    def mainloop(self):
        while 1:
            self.screen.fill((35,38,117))
            self.make_text(397, 50, 'snakegame by thajudheen', color = (255,255,255), size = 40, a = True)
            self.make_text(312, 130, 'GitHub: https://github.com/thajudecodes', color = (255,255,255), size = 20, a = True)
            self.make_text(348, 190, 'Instagram: https://instagram.com/_.thaju____', color = (255,255,255), size = 20, a = True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
